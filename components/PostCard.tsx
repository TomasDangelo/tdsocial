...
function PostCard({post, dbUserId}: {post: Post, dbUserId: string | null}) {
  ...
  const handleLike = async() =>{
    ...
    try {
      ...
      setOptimisticLikes(post._count.likes)  //Volvemos a los estados por default si hay error
  ...
  const handleDeletePost = async() =>{
    ...
    try {
      ...
      if(result?.success) toast.success("Post eliminado exitosamente")
      else throw new Error("Error eliminando post")
    } catch (error) {
...
const handleDeleteComment = async (id: string) => {
  ...
  try {
    ...
    if(result?.success) toast.success("Comentario eliminado exitosamente")
      else throw new Error("Error eliminando comentario")
  } catch (error) {
    ...
    <Card className="overflow-hidden">
      <CardContent className="p-4 sm:p-6">
        <div className="space-y-4">
          ...
          {/* BOTONES DE COMENTARIO Y ME GUSTA */}
              ...
              {user ? (
                <div className="flex space-x-3">
                  ...
                  <div className="flex-1">
                    <Textarea placeholder="Escribí un comentario..." value={newComment} onChange={(e) => setNewComment(e.target.value)} className="min-h-[80px] resize-none"
                    />
